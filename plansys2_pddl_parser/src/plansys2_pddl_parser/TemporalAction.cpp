...
Expression * TemporalAction::parseDuration(
  Stringreader & f, TokenStruct<std::string> & ts,
  Domain & d)
{
...
void TemporalAction::printCondition(
  std::ostream & s, const TokenStruct<std::string> & ts, const Domain & d,
  ...
  const TokenStruct<std::string> & ts, const Domain & d) const
{
  ...
  s << "  :duration ( = ?duration ";
  if (durationExpr) {durationExpr->PDDLPrint(s, 0, astruct, d);} else {s << "1";}
  s << " )\n";
