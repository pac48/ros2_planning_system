...
public:
      ...
      {
        if (t == "requirements") {parseRequirements(f);} else if (t == "types") {
          parseTypes(f);
        } else if (t == "constants") {parseConstants(f);} else if (t == "predicates") {
          parsePredicates(f);
        } else if (t == "functions") {parseFunctions(f);} else if (t == "action") {
          parseAction(f);
        } else if (t == "durative-action") {parseDurativeAction(f);} else if (t == "derived") {
          parseDerived(f);
        }
//		else if ( t == "axiom" ) parseAxiom( f );
      ...
      {
        if (s == "strips") {strips = true;} else if (s == "adl") {
          adl = true;
        } else if (s == "negative-preconditions") {
          neg = true;
        } else if (s == "conditional-effects") {condeffects = true;} else if (s == "typing") {
          typed = true;
        } else if (s == "action-cost") {costs = true;} else if (s == "equality") {
          equality = true;
        } else if (s == "durative-actions") {temp = true;} else if (s == "non-deterministic") {
          nondet = true;
        } else if (s == "universal-preconditions") {universal = true;} else if (s == "fluents") {
          fluents = true;
...
//		if ( costs ) insert( new Type( "NUMBER" ), tmap, types );
      ...
      void addPre(
        bool neg, const std::string & act, const std::string & pred,
        const IntVec & params = IntVec() )
      {
        Action * action = actions.get(act);
        if (action->pre == 0) {action->pre = new And;}
        And * a = dynamic_cast < And * > ( action->pre );
        if (neg) {a->add(new Not(ground(pred, params) ) );} else {a->add(ground(pred, params) );}
      }
      ...
      void addCost(
        const std::string & act, const std::string & func,
        const IntVec & params = IntVec() )
      {
        Action * action = actions.get(act);
        if (action->eff == 0) {action->eff = new And;}
