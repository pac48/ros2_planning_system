...
public:
      ...
      void parse(const std::string & s)
      {
        ...
        for ( ; f.getChar() != ')'; f.next() ) {
          ...
          if (t == "domain") {parseDomain(f);} else if (t == "objects") {
            parseObjects(f);
          } else if (t == "init") {parseInit(f);} else if (t == "goal") {
            parseGoal(f);
          } else if (t == "metric") {parseMetric(f);} else {f.tokenExit(t);}
        }
      }
      ...
      virtual void parseGround(Stringreader & f, GroundVec & v)
      {
        ...
        if (f.getChar() == '=') {
          ...
          if (d.funcs[i]->returnType < 0) {c = new GroundFunc < double > ( d.funcs[i] );} else {
      ...
      void parseMetric(Stringreader & f)
      {
        ...
        f.assert_token("(");
        if (d.temp) {f.assert_token("total-time");} else {f.assert_token("total-cost");}
        f.assert_token(")");
        f.assert_token(")");
      }
      ...
      void addInit(TypeGround * g, const StringVec & v = StringVec() )
      {
        ...
        if (f1) {tg = new GroundFunc < int > ( d.funcs.get(g->name), f1->value );} else if (f2) {
      ...
      friend std::ostream & operator <<
        (std::ostream & os, const Instance & o) {return o.print(os);}
        virtual std::ostream & print(std::ostream & stream) const
          ...
          if (metric) {
            ...
            if (d.temp) {stream << "time";} else {stream << "cost";}
            stream << " ) )\n";
          }
