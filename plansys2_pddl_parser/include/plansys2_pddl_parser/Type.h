...
public:
      ...
      std::pair < bool, int > parseConstant(const std::string & object) {
        ...
        if (i < 0) {k += constants.size();} else {return std::make_pair(true, -1 - i);}

        for (unsigned i = 0; i < subtypes.size(); ++i) {
          std::pair < bool, int > p = subtypes[i]->parseConstant(object);
          if (p.first) {return std::make_pair(true, -k + p.second);} else {k += p.second;}
        }
      ...
      std::pair < bool, unsigned > parseObject(const std::string & object) {
        ...
        if (i < 0) {k += objects.size();} else {return std::make_pair(true, i);}
      ...
      std::pair < std::string, int > object(int index) {
        if (index < 0) {
          ...
          if (index < (int)objects.size() ) {return std::make_pair(objects[index], 0);} else {
            index -= objects.size();
          }
        }
        ...
        for (unsigned i = 0; i < subtypes.size(); ++i) {
          ...
          if (p.first.size() ) {return p;} else {index = p.second;}
        }
