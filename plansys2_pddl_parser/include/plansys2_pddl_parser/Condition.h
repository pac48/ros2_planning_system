...
public:
      UnsupportedConstruct(const std::string & construct) : std::runtime_error(
          construct + " is not currently supported by plansys2") {
      }
    };
...
public:
      ...
      virtual void PDDLPrint(
        std::ostream & s, unsigned indent,
        const TokenStruct < std::string > & ts, const Domain & d) const = 0;
